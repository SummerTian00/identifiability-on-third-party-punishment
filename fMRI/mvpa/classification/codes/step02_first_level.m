...
for sub_num=1:length(img_folders)
    ...
    mkdir(path,filesep,'first_level_MVPA');
    out_path=[path,filesep,'first_level_MVPA'];
    ...
    run1_vector=[path ,filesep,'conditions_' run_folder(1).name '_MVPA.mat'];
    run1_vector=cellstr(run1_vector);
    run2_vector=[path,filesep,'conditions_' run_folder(2).name '_MVPA.mat'];
