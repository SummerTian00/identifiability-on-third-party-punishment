...
for sub_num=1:length(img_folders)
    ...
    mkdir([path,filesep,'first_level_uniact']);
    out_path=[path,filesep,'first_level_uniact'];
    
    
    run_folder=dir([path,filesep,'ge_func_3p5x3p5x3p5_270*']);
    run1_file=spm_select ('FPList',[path,filesep,run_folder(1).name,filesep], '^swuaf.*\.nii');
    run1_file= cellstr(run1_file);
    
    run2_file=spm_select ('FPList',[path,filesep,run_folder(2).name,filesep], '^swuaf.*\.nii');
    run2_file= cellstr(run2_file);
    
    run1_vector=[path,filesep,'conditions_' run_folder(1).name  '_uniact.mat'];
    run1_vector=cellstr(run1_vector);
    
    run2_vector=[path,filesep,'conditions_' run_folder(2).name  '_uniact.mat'];
    run2_vector=cellstr(run2_vector);
    
    con_name={'01unide','02unide_p','03ide','04ide_p','05p'};
    con_weight=zeros(5,4);
    con_weight(1,:)=[1,0,0,0]; %unide
    con_weight(2,:)=[0,1,0,0]; %unide_p
    con_weight(3,:)=[0,0,1,0]; %ide
    con_weight(4,:)=[0,0,0,1]; %ide_p
    con_weight(5,:)=[0,1,0,1]; %p
    %% -----------------------------------------------------------------------
    ...
    for con_len=1:size(con_weight,1)
