// The impartial model,additive version: this model assumes that participants act as a judge, while contexts impacts threshold of inequality aversion.
  ...
  for (i in 1:N) {
    for (t in 1:Tsubj[i]) {
      ...
      if (cond[i, t]==1) { // ambigous cond
          util[1] = 6-0-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*0-omega_a[i]);
          util[2] = 6-2-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*2-omega_a[i]);
          util[3] = 6-4-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*4-omega_a[i]);
          util[4] = 6-6-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*6-omega_a[i]);
      } else { // identified cond
          util[1] = 6-0-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*0-omega_i[i]);
          util[2] = 6-2-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*2-omega_i[i]);
          util[3] = 6-4-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*4-omega_i[i]);
          util[4] = 6-6-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*6-omega_i[i]);
      }
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        if (cond[i, t]==1) { // ambigous cond
            ...
            // Model regressors
            //Uc[i,t] = 6-choice[i, t]-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*choice[i, t]-omega_a[i]);
            ...
            // Model regressors
            //Uc[i,t] = 6-choice[i, t]-alpha[i]*fabs(xd[i,t]-xv[i,t]-2*choice[i, t]-omega_i[i]);
        }
