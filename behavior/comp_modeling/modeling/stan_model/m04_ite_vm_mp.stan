// The vicarious model,multiplicative version: this model assumes that participants take the perspective of victim, while contexts impacts sensitivity to inequality aversion.
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if (cond[i, t]==1) { // ambigous cond
          util[1] = 6-0-alpha_a[i]*fmax(xd[i,t]-xv[i,t]-2*0,0);
          util[2] = 6-2-alpha_a[i]*fmax(xd[i,t]-xv[i,t]-2*2,0);
          util[3] = 6-4-alpha_a[i]*fmax(xd[i,t]-xv[i,t]-2*4,0);
          util[4] = 6-6-alpha_a[i]*fmax(xd[i,t]-xv[i,t]-2*6,0);
      } else { // identified cond
          util[1] = 6-0-alpha_i[i]*fmax(xd[i,t]-xv[i,t]-2*0,0);
          util[2] = 6-2-alpha_i[i]*fmax(xd[i,t]-xv[i,t]-2*2,0);
          util[3] = 6-4-alpha_i[i]*fmax(xd[i,t]-xv[i,t]-2*4,0);
          util[4] = 6-6-alpha_i[i]*fmax(xd[i,t]-xv[i,t]-2*6,0);
      }
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      if (cond[i, t]==1) { // ambigous cond
          ...
          //Uc[i,t] = 6-choice[i, t]-alpha_a[i]*fmax(xd[i,t]-xv[i,t]-2*choice[i, t],0);
          ...
          //Uc[i,t] = 6-choice[i, t]-alpha_i[i]*fmax(xd[i,t]-xv[i,t]-2*choice[i, t],0);
      }
