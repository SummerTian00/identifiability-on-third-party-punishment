...
%% This script is used to normalize and smooth dprime images after MVPA analysis.
%-----------------------------------------------------------------------
spm('defaults', 'FMRI');
...
for i=1:length(folders)
    ...
    for j=1:length(MVPAfolder)
        dprime_folder=dir([path,filesep, sub_file,filesep,MVPAfolder(j).name,filesep,'*_searchlight']);
        
        for xx=1:length(dprime_folder)
            
            dprime_image=spm_select ('FPList',[path,filesep,sub_file,filesep,MVPAfolder(j).name,filesep,dprime_folder(xx).name,filesep], '^res.*\.nii');
            ...
            for yy=1:length(dprime_image)
                ...
                matlabbatch{1}.spm.spatial.coreg.estimate.source = dprime_image(yy);
                ...
                matlabbatch{2}.spm.spatial.normalise.write.subj.resample(1) =  dprime_image(yy);
